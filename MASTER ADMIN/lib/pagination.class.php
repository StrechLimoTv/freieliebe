...
class pagination {
	...
	function __construct($total, $page, $perpage = 10){
		...
		if($page > 1)
			$this->pages .= "<li><a href='?page=".($page-1)."$query'>Zurück</a> </li>";

		for($i = max(1, $page - 3); $i <= min($page + 3, $total_pages); $i++)
			$this->pages .= ($i == $page ? "<li class='active'><a>".$i."</a></li>" : " <li><a href='?page=$i$query'>$i</a></li> ");

		if($page < $total_pages)
			$this->pages .= "<li><a href='?page=".($page+1)."$query'>Weiter</a></li>";

		if($page < $total_pages-3)
			$this->pages .= "<li><a href='?page=$total_pages$query'> Zuletzt </a></li>";
